...
//eeessaaa
	...
	/*
	 * Write a non-member method named addUpToN that has two parameters: a list of
	 * integer values and a single integer value N. The method returns a list with
	 * the first two values found in the list that add up to N, and an empty list if
	 * no such numbers exist. We want an implementation that runs efficiently, so
	 * you cannot sort the input list and you cannot use a nested for-loop; you MUST
	 * use a Hash table. Hint: For every number x you consider from the input list,
	 * you must be able to determine whether you had previously seen the number N -
	 * x. Hash tables are good for keeping track of things you've seen...
	 */
	public static AddUpToNWrapper.List<Integer> addUpToN(AddUpToNWrapper.List<Integer> input, int N) {

		List<Integer> neue = new SinglyLinkedList<>();
		Map<Integer, Integer> table = new HashtableSC<Integer, Integer>();
		int d = 0;
		if (input.isEmpty()) {
			return neue;
		}
		for (int i = 0; i < input.size(); i++) {
			table.put(input.get(i), input.get(i));
		}
		for (int i = 0; i < input.size(); i++) {
			d = N - input.get(i);

			if (table.contains(d)) {
				neue.add(input.get(i));
				neue.add(table.get(d));
				return neue;
